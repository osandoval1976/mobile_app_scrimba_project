...
var ImageHandler = class {
  ...
  #logger;
  #originServerURL;
  constructor({ logger, imagesConfig, originServerAddress }) {
    this.#logger = logger;
    ...
    this.#originServerURL = originServerAddress ? new URL(originServerAddress) : void 0;
  }
    ...
    return {
      handle: async () => {
        ...
        try {
          ...
          throw new Error(
            `Failed to construct source image URL from "${sourceImageUrlParam}".` + (!this.#originServerURL && !sourceImageUrlParam.startsWith("http") ? "\nLooks like source image is local and `originServerAddress` was not provided." : ""),
            { cause: error }
          );
        }
        if (sourceImageUrl.origin !== this.#originServerURL?.origin && !this.#allowedRemoteUrlPatterns.some(
